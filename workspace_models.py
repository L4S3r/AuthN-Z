"""
Auth N&Z - Workspace/Task/Team Domain Models (workspace_models.py)
--------------------------------------------------------------------
These models are specific to the turnkey standalone server's multi-tenant
workspace + sprint task tracker feature set. They are NOT part of the BYOU
(Bring-Your-Own-User) surface - a host app that only wants auth/JWT/guards
never imports this module, so its own schema never gets coupled to
workspaces/tasks/notifications.

Moved out of models.py verbatim (no logic changes) as part of separating
"core identity" (models.py, default_user.py) from "turnkey app domain".
"""
